...
You want weights to be bundled in the Cog image. If the model you're cogifying is using weights from the HuggingFace hub, you want to download them to the local workspace before pushing them to Replicate. That can be achieved by setting the HF_HOME variable to `/src/hf_home` just after loading the os module, before loading any other modules (this is important!).
...
import os
os.environ["HF_HOME"] = "/src/hf_home"  # <- this is important, it needs to happen before importing any modules!
